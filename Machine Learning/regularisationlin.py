...
def cost_func_l1(theta, X, y,lamb):
    m = len(X)
    pred = X.dot(theta)
    cost = (1/m) * (np.sum(np.square(pred - y))+(lamb*(np.sum(np.abs(theta)))))
    return cost**(1/2)

def gradientDescent(X_train, X_test, y_train, y_test, learning_rate=0.1, iteration=1000, lamb=0):
    m = len(y_train)
    ...
    for i in range(iteration):
        ...
        theta = theta - (1 / m) * learning_rate * (X_train.T.dot((pred - y_train))+(lamb*(np.sign(theta))))
        rmse_train[i] = cost_func_l1(theta, X_train, y_train,lamb)
        rmse_test[i] = cost_func_l1(theta, X_test, y_test,lamb)
